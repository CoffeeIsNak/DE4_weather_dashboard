...
# 데이터 받아오는 API 경로
BASE_URL = 'https://apihub.kma.go.kr/api/typ01/url/fct_afs_wl.php'
...
def fetch_last_date():
    ...
    with engine.connect() as connection:
        result = connection.execute(text("SELECT MAX(TM_FC) FROM Medium_term_forecast_from_ground"))
...
def save_to_mysql(data):
    # 데이터프레임 생성
    col_name = ['REG_ID', 'TM_FC', 'TM_EF', 'MOD', 'STN', 'C', 'SKY', 'PRE', 'CONF', 'WF', 'RN_ST']
    df = pd.DataFrame([line.split(',')[:-1] for line in data], columns=col_name)

    df['CreateAt'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

    df.to_sql(name='Medium_term_forecast_from_ground', con=engine, if_exists='append', index=False)
